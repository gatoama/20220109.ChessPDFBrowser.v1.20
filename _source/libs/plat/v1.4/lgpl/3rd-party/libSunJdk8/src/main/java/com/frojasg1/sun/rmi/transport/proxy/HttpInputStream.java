...
class HttpInputStream extends FilterInputStream {
   ...
   public HttpInputStream(InputStream var1) throws IOException {
      ...
      do {
         ...
         if (com.frojasg1.sun.rmi.transport.proxy.RMIMasterSocketFactory.proxyLog.isLoggable(Log.VERBOSE)) {
            com.frojasg1.sun.rmi.transport.proxy.RMIMasterSocketFactory.proxyLog.log(Log.VERBOSE, "received header line: \"" + var5 + "\"");
         }
      ...
      if (com.frojasg1.sun.rmi.transport.proxy.RMIMasterSocketFactory.proxyLog.isLoggable(Log.VERBOSE)) {
         com.frojasg1.sun.rmi.transport.proxy.RMIMasterSocketFactory.proxyLog.log(Log.VERBOSE, "content length: " + this.bytesLeft);
      }
   ...
   public int read() throws IOException {
      if (this.bytesLeft > 0) {
         ...
         if (com.frojasg1.sun.rmi.transport.proxy.RMIMasterSocketFactory.proxyLog.isLoggable(Log.VERBOSE)) {
            com.frojasg1.sun.rmi.transport.proxy.RMIMasterSocketFactory.proxyLog.log(Log.VERBOSE, "received byte: '" + ((var1 & 127) < 32 ? " " : String.valueOf((char)var1)) + "' " + var1);
         }

         return var1;
      } else {
         com.frojasg1.sun.rmi.transport.proxy.RMIMasterSocketFactory.proxyLog.log(Log.VERBOSE, "read past content length");
         return -1;
      }
   }
