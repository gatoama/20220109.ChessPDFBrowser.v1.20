...
{
	protected static final double VALUE = 3d;
	public static final String PIECE_TYPE_CODE="N";

	public Knight( ChessBoard chessBoard, int color )
	{
	...
	public boolean isProbablyLegalThisMove_child( ChessGameMove cgm, boolean weKnowIfIsACaptureMoveOrNot ) throws ChessMoveException
	{
		...
		int adc = IntegerFunctions.abs( cgm._deltaColumn );
		int adr = IntegerFunctions.abs( cgm._deltaRow );
		
		result = ( adc == 2 ) && ( adr == 1 ) || ( adc == 1 ) && ( adr == 2 );
